;; ****
;; WE WERENT ABLE TO FINISH ALL OF THE FUNCTIONALITY BUT WE HAVE IMPLEMENTED 1,2,3,5,6,7. WE IMPLEMENTED THE ABILITY TO CREATE NEW INSTANCES, EVALUATE FUNCTION CALLS, EVALUATE VARIABLE CALLS
;; OUR MAJOR PROBLEM IS THAT WE HAVE NOT IMPLEMENTED "THIS" TO BE PASSED TO ALL MVALUE/MSTATE FUNCTIONS
;; To add "this" to all mstate and mvalue functions we would:
;; -Add a field called this to mvalue-helper that would be returned as "this" in the third case
;; -Add a this as a parameter to Mvalue, so it could be passed into mvalue-helper
;; -Add this as a parameter to all function calls
;; -Add this as a parameter to all other mstate mvalue functions (could not get them all working and was breaking code)
;; ****
  ...
  (lambda (filename class)
    (scheme->language
     (func_run
      'main
      '()
      (list (class-body (lookup (string->symbol class) (list (interpret_class_bodies (interpret_classes_next (interpret_classes (parser filename) '((()())))))) )))
      (lambda (v env) v)
      (list (interpret_class_bodies (interpret_classes_next (interpret_classes (parser "test.txt") '((()()))))))))))
...
;; Evaluates a funcall expression using the correct instances and class methods
;; THIS FUNCTION HANDLES DOT CALLS
  ...
  (lambda (func_call func_params state this throw [classes 'null])
    (let ((func_closure 
           (cond
             ...
             [(list? (cadr func_call)) (lookup (car (cddadr func_call)) (list (class-body (lookup this classes))))]
             [else 'nothing])))
...
(define mvalue
  (lambda (expr environment cps-return throw [classes 'null])
    (cond
      ...
      [(eq? (operator expr) 'dot)        (cps-return (lookup-in-frame (caddr expr) (cddr (lookup (cadr expr) environment))))]
...
;; returns the scoped state of the function
...
; returns the this values of the expression
...
;; Creates the closures of the global classes
...
;; these functions Interpret the bodies of all of the global classes
(define interpret_class_bodies
  (lambda (classes)
    (cons (caar classes) (list (interpret_class_bodies_1 (cadar classes) (caar classes))))))

(define interpret_class_bodies_1
  (lambda (classes names)
    (cond
      [(eq? classes '()) '()]
      [(cons (box (cons (car (unbox (car classes))) (list (interpret_class_bodies_2 (cadr (unbox (car classes))) (car names) )))) (interpret_class_bodies_1 (cdr classes) (cdr names)))])))

(define interpret_class_bodies_2
  (lambda (body cur_type)
    (car (interpret_class body '((()())) cur_type) )))
...
;; creates and instance at the class level
...
;These functions parse the classes for all of its fields and values
...
(define interpret_class
  (lambda (parse_tree state cur_type)
    (cond
      [(null? parse_tree) state]
      [(eq? (first_stmt_type parse_tree) 'var) (interpret_class (remaining_tree parse_tree) (interpret-global-declare (first_stmt parse_tree) state) cur_type)] ;global variable declaration
      [(eq? (first_stmt_type parse_tree) 'function) (interpret_class (remaining_tree parse_tree) (func_define (function_declaration parse_tree) state cur_type) cur_type)] ;global function declaration
      [(eq? (first_stmt_type parse_tree) 'static-function) (interpret_class (remaining_tree parse_tree) (func_define (function_declaration parse_tree) state cur_type) cur_type)]
...
(define func_define
  (lambda (function state [cur_type 'null])
    (func_new_environment function (new_state_initializer function state) cur_type)))

;Updates an initialized functions value in the state with the correct function closure
(define func_new_environment
  (lambda (function environment cur_type)
    (update (func_name function) (create_closure (func_params function) (func_body function) environment cur_type) environment)))
...
(define create_closure
  (lambda (params body func_environment cur_type)
    (append (cons params (cons body (cons func_environment '()))) (list cur_type))))

;; for implementing static-non-static code
